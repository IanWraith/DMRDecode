...
public class FullLinkControl {
	...
	private String decodeServiceOptions (boolean bits[],int offset)	{
		int priority;
		StringBuilder sb=new StringBuilder(250);
		sb.append("<b>Service Options : ");
		// Emergency
		if (bits[offset]==false) sb.append("Non-emergency");
		else sb.append("Emergency");
		// Privacy
		if (bits[offset+1]==true) sb.append("/Privacy Enabled");
		// +2 and +3 are reserved bits
		// +4 is Broadcast
		if (bits[offset+4]==true) sb.append("/Broadcast");
		// +5 is OVCM
		if (bits[offset+5]==true) sb.append("/OVCM Call");
